...
public class WeightLoadBalancer extends LoadBalancer {
    ...
    public static final Attributes.Key<Set<String>> WRITE_LIST_KEY = Attributes.Key.create("grpc_write_list");
    ...
    static final class ReadyPicker extends WeightLoadBalancer.WeightPicker {
        ...
        private Subchannel nextSubchannel() {
            try {
                for (Subchannel subchannel : list) {
                    Set<String> writeList = subchannel.getAttributes().get(WRITE_LIST_KEY);
                    if (writeList != null && !writeList.isEmpty()) {
                        var userIP = GrpcLoadBalancerContext.getAttribute(GrpcLoadBalanceAttributes.USER_IP_KEY)
                            .orElse(null);
                        if (userIP != null && writeList.contains(userIP)) {
                            return subchannel;
                        }
                    }
                ...
                GrpcLoadBalancerContext.lazyClear();
            }
        }
         ...
         */
        private int rand(final int from, final int to) {
            if (to < from) {
