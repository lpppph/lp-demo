                    ...
                    for (ServiceInstance serviceInstance : serviceInstanceList) {
                        ...
                        if (metadata.get("gRPC.port") != null) {
                            ...
                            if (metadata.get(WeightLoadBalancer.WEIGHT_KEY.toString()) != null) {
                                ...
                                logger.info("Found gRPC server {} {} {}", name, WeightLoadBalancer.WEIGHT_KEY.toString(), weight);
                            }
                            if (metadata.get(WeightLoadBalancer.WRITE_LIST_KEY.toString()) != null) {
                                String writeList = metadata.get(WeightLoadBalancer.WRITE_LIST_KEY.toString());
                                try {
                                    String[] urls = writeList.split("\\|");
                                    builder.set(WeightLoadBalancer.WRITE_LIST_KEY, Sets.newHashSet(urls));
                                } catch (Exception e) {
                                    logger.error("can not parsing write list", writeList);
                                }
                                logger.info("Found gRPC server {} {} {}", name, WeightLoadBalancer.WRITE_LIST_KEY.toString(), writeList);
                            }
            ...
            for (ServiceInstance serviceInstance : serviceInstanceList) {
                ...
                for (ServiceInstance newServiceInstance : newServiceInstanceList) {
                    if (newServiceInstance.getHost().equals(serviceInstance.getHost())
                        ...
                        Map<String, String> oldMetadata = serviceInstance.getMetadata();
                        if (newMetadata.size() == oldMetadata.size()) {
                            boolean isMetaDataSame = true;
                            for (Map.Entry<String, String> entry : newMetadata.entrySet()) {
                                String value = oldMetadata.get(entry.getKey());
                                if (!entry.getValue().equals(value)) {
                                    isMetaDataSame = false;
                                    break;
                                }
                            }
                            if (isMetaDataSame) {
